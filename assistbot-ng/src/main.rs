...
use commands::util::message::MbcCommon;
use commands::util::slash::CirmResult;
use commands::util::CommandError;
use commands::util::{slash::ScCommon, slash::SlashCommand, Common};
use commands::MESSAGE_BASED_COMMANDS;
use commands::SLASH_COMMANDS;
use reqwest::Client as HttpClient;
use serenity::all::CreateMessage;
use serenity::all::{
    Command, CreateEmbed, CreateEmbedFooter, CreateInteractionResponse,
    CreateInteractionResponseMessage, Interaction,
};
use serenity::async_trait;
use serenity::model::{channel::Message, gateway::Ready};
use serenity::prelude::*;
use shuttle_runtime::{Error as shuttle_error, SecretStore};
use std::sync::Arc;
...
impl<'a> EventHandler for Bot<'static> {
    async fn interaction_create(&self, ctx: Context, interaction: Interaction) {
        if let Interaction::Command(ref command) = interaction {
            ...
            let mut msg: CirmResult = Err(CommandError::Default(
                "Command dosent found".to_string(),
            ));
                ...
                msg = command_proc
                    .run(
                        ...
                        &ScCommon {
                            ...
                            http_client: self.common.http_client.to_owned(),
                        },
                    )
                    .await;
                break;
            ...
            if let Err(err) = msg {
                let embed = match err {
                    CommandError::Default(v) => CreateEmbed::new_from_settings()
                        .title("Error")
                        .description(v),
                    CommandError::Argument(name, msg) => CreateEmbed::new_from_settings()
                        .title("Error")
                        .description(msg)
                        .footer(CreateEmbedFooter::new(format!(
                            "Caused by argument '{name}'"
                        ))),
                };
    ...
    async fn message(&self, ctx: Context, msg: Message) {
        ...
        let content_lower = content.to_owned().to_lowercase();
        ...
        for command_raw in MESSAGE_BASED_COMMANDS.iter() {
            ...
            if content_lower.starts_with(&format!(
                "{}{}",
                settings::MESSAGE_BASED_COMMANDS_PREFIX,
                command.name()
            )) {
                let res = command
                    .run(
    ...
    async fn ready(&self, ctx: Context, ready: Ready) {
        ...
        ctx.set_presence(Some(activity.to_owned()), status);
        // Register all commands and store their mapping
        for command_opt in SLASH_COMMANDS.iter() {
            if let Some(command) = command_opt {
                let gc_raw = Command::create_global_command(&ctx.http, command.register()).await;
...
async fn serenity(
    ...
    if !settings::DISABLE_MESSAGE_BASED_COMMANDS
        && !intents_from_settings.contains(&GatewayIntents::GUILD_MESSAGES)
    {
    ...
    if client.is_err() {
        return Err(shuttle_error::BuildPanic(
            "Failed to make Serenity client".to_owned(),
        ));
    }
